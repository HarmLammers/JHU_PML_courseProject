...
The original training dataset will be split into 3; 60% for training, 30% for cross-validation and 10% for test purposes.  
...
Based on the results we see a perfect fit on the cross-validation-set with the resulting Random Forest model based on the trainig set. GBM comes close to this result but I choose the RF-model as the resulting model. 

In order to predict the out-of-sample error I use the 3d subset of the trainingdata as a testset. Though the results have been calculated for the other two models, I will not report them in the presentation.
...
Even in the test-set I get a 100% score which leaves me with a 95% confidence interval for Acccuracy: (0.9958, 1).
